...
class alphamodel():
    ...
    def chi_2_lvl(self):
        return self.Gamma_e*(self.Gamma_e+self.gamma_eg)/((self.Gamma_e+self.gamma_eg)**2+2*self.Omega_p**2*(self.Gamma_e+self.gamma_eg)/self.Gamma_e)
    #def chi_3_lvl(self):
    ...
    def integrand(self):
        return chi_0*self.ground_state_density()*self.f_ir()*self.chi_2_lvl()*((self.chi_3_lvl()/self.chi_2_lvl()+self.f_bl())/(1+self.f_bl())-1)
    def alpha(self):
    ...
    def min_counts_raw(self):
        return self.min_counts_e()/self.Q_E*np.e**(integrate.simps(chi_0*self.ground_state_density()*(self.chi_3_lvl()+self.chi_2_lvl()*self.f_bl())/(1+self.f_bl()),self.z_grid))
    def transmission_simple_eit(self):
        return np.e**(integrate.simps(-chi_0*self.ground_state_density()*(self.chi_3_lvl()+self.chi_2_lvl()*self.f_bl_simple())/(1+self.f_bl_simple()),self.z_grid))
    def transmission_eit(self):
        return np.e**(integrate.simps(-chi_0*self.ground_state_density()*(self.chi_3_lvl()+self.chi_2_lvl()*self.f_bl())/(1+self.f_bl()),self.z_grid))
